...
public function showMyBlogs(){
    $userId=auth()->id();
    $score=Score::where('user_id',$userId)->pluck('score')[0];
...
public function filterBlog(Request $request){
    ...
    $blog=Blog::query();
    if($categoryIds){
        $blog->whereHas('category',function($query) use($categoryIds){
            $query->whereIn('categories.id',$categoryIds);
        });
    }
    if($tagIds){
        $blog->whereHas('tag',function($query) use($tagIds){
