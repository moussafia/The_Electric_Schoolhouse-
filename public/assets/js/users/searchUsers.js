$(document).ready(function(){ 
    $('#default-search').on('input',function(){
        ...
        $.ajax({
            ...
            dataType:'json',
            cache: false,
            success: function (response) {console.log(response.users.roles);
                var html="";
                $.each(response.users, function (index, users) {
                    html+=`<div class="px-3 py-2 cursor-pointer rounded-lg" style="border-bottom-width: 1px;">
                                ...
                                document.getElementById('searchBtn2').setAttribute('data-Name', '${users.first_name} ${users.last_name}');
                                document.getElementById('searchBtn2').setAttribute('data-IdUser', '${users.id}');
                                document.getElementById('searchBtn2').click();">
                                        ...
                                        <span class="font-bold text-md">${users.first_name} ${users.last_name} </span>
                                </div>
                                <span class="text-sm pl-5" style="font-family: cursive">
                                ${users.roles.map(role=>role.name)}</span>
                            </div> `;
...
$('#searchBtn2').on('click',function(){
    var UsersName=$('#searchBtn2').attr('data-Name');
    var idUser=$('#searchBtn2').attr('data-IdUser');
     $('#default-search').val(UsersName);
    $('#idUser').val(idUser);
    $('#searchResults').empty();
    
})
$(document).ready(function() {
    $('#barSearchUsers').on('submit',function(e){
        e.preventDefault();
        var query=$('#default-search').val().trim().split(' ');
        if(query[1]===''){
            return;
        }
        $.ajax({
